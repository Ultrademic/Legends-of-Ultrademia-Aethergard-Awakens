// /modules/zones/zone_elven_ruins.js
// ELVEN RUINS — Mid-tier starter zone for Ultrademia
// Synced with new MonsterStats + spawning + combat + player system.
...
export async function loadElvenRuins() {
    ...
    // -------------------------------------------------
    // CLEAR SCENE (keep player & camera & particles)
    // -------------------------------------------------
    const toRemove = [];
    for (const obj of scene.children) {
        if (obj === player.group || obj === camera) continue;
    ...
    // -------------------------------------------------
    // CENTRAL PLATFORM (ancient elven circle)
    // -------------------------------------------------
    ...
    // -------------------------------------------------
    // MONSTERS: RUINS WOLVES
    // Mid-tier compared to village wolves
    // -------------------------------------------------
    const wolfConfig = {
        family: "wolf",
        name: "Ruins Wolf",
    ...
    const checkPortal = () => {
        ...
        if (dist < 2.2) {
            console.log("Portal triggered → Village");
            loadVillage();
        }
    };
    ...
    console.log("%c[Zone] Elven Ruins ready.", "color:lime");
}

...
export function animateFallbackHumanoid(entity, dt, state) {
    if (!entity.userData.isFallback) return;
    
    entity.userData._time = (entity.userData._time || 0) + dt;
    const clips = entity.animations; 
    
    if (!clips) return;
    
    // Map logical state from StateMachine to fallback clip key
    const map = {
        idle: 'idle',
        moving: 'run',
        attack: 'attack',
    ...
    const key = map[state] || 'idle';
    const clip = clips[key];
    
    if (clip && clip.update) {
        clip.update(entity, entity.userData._time);
    }
}